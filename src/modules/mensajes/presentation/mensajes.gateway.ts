...
export class MensajesGateway {
  ...
  async handleIntegranteEliminado(payload: {
    ...
    const { id_chat, miembro_eliminado, id_miembro_eliminado, chat_actualizado } = payload;
    ...
    this.appGateway.emitToUser(id_miembro_eliminado, TipoEvento.INTEGRANTE_ELIMINADO, {
      id_chat,
      miembro_eliminado,
  ...
  async handleGrupoEliminado(payload: {
    id_chat: string;
    ...
    this.appGateway.emitToUsers(usuarios_afectados, TipoEvento.GRUPO_ELIMINADO, {
      id_chat,
      nombre_grupo,
